function dijkstra(cells, row, col){
	...
	while(queue.length > 0 && foundPath == null){
		...
		var path = popCheapestPath(queue);
...
/**
 *	returns the cheapest path in the queue and removes it from the queue
 * 	much like priority queues
 */
function popCheapestPath(queue){
